...
Agenda.prototype.saveJob = function (job, cb) {
    ...
    if (id) {
        ...
        // Try an upsert.
        this._collection.findAndModify({name: props.name, type: 'single'}, {}, update, {
            upsert: true,
            new: true
        }, processDbResult);
    } else if (unique) {
...
Agenda.prototype._findAndLockNextJob = function (jobName, definition, cb) {
    ...
    if (this._mdb.readyState != 1) { // no active connection
...
function processJobs(extraJob) {
    ...
    if (!extraJob) {
        ...
        var now = new Date();
        self._collection.findAndModify({
            _id: extraJob.attrs._id,
            lockedAt: null,
            disabled: {$ne: true}
        }, {}, {$set: {lockedAt: now}}, {new: true}, function (err, resp) {
